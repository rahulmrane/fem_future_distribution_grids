...
function initialise_params(mesh)
    ...
    return sourceperelement, reluctivityperelement, conductivityperelement, omega, coil_voltage, ext_resistance, ext_inductance, z_length, time_steps, har_content, har_phase, core_elements
end
...
sourceperelement, reluctivityperelement, conductivityperelement, omega, coil_voltage, ext_resistance, ext_inductance, z_length, time_steps, har_content, har_phase, core_elements = initialise_params(mesh)
...
xlabel!("time (sec)")
ylabel!("u")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_u_OC.png")

plot(time_steps, Bx_wave, label = "Bx")
xlabel!("time (sec)")
ylabel!("Bx")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_Bx_OC.png")

plot(time_steps, By_wave, label = "By")
xlabel!("time (sec)")
ylabel!("By")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_By_OC.png")

plot(time_steps, B_wave, label = "u")
xlabel!("time (sec)")
ylabel!("B")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_B_OC.png")

plot(time_steps, Hx_wave, label = "Hx")
xlabel!("time (sec)")
ylabel!("Hx")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_Hx_OC.png")

plot(time_steps, Hy_wave, label = "Hy")
xlabel!("time (sec)")
ylabel!("Hy")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_Hy_OC.png")

plot(time_steps, H_wave, label = "H")
xlabel!("time (sec)")
ylabel!("H")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_H_OC.png")

plot(time_steps, mag_energy_wave, label = "energy")
xlabel!("time (sec)")
ylabel!("energy")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_energy_OC.png")

plot(time_steps, reluctivityperelement_wave, label = "mur")
xlabel!("time (sec)")
ylabel!("mur")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_mur_OC.png")

plot(time_steps, Jel_wave, label = "Jel")
xlabel!("time (sec)")
ylabel!("Jel")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_Jel_OC.png")

plot(time_steps, Pv_wave, label = "Pv")
xlabel!("time (sec)")
ylabel!("Pv")
savefig((@__DIR__) * "/../img/3_ph_stedin_trafo_Pv_OC.png")
...
# CSV.write((@__DIR__) * "/../csv/3_ph_stedin_trafo_Bx_OC.csv", Bx_mat)
# By_mat = DataFrame(hcat(By...), :auto)
# CSV.write((@__DIR__) * "/../csv/3_ph_stedin_trafo_By_OC.csv", By_mat)
# B_mat = DataFrame(hcat(B...), :auto)
# CSV.write((@__DIR__) * "/../csv/3_ph_stedin_trafo_B_OC.csv", B_mat)
# Hx_mat = DataFrame(hcat(Hx...), :auto)
# CSV.write((@__DIR__) * "/../csv/3_ph_stedin_trafo_Hx_OC.csv", Hx_mat)
# Hy_mat = DataFrame(hcat(Hy...), :auto)
# CSV.write((@__DIR__) * "/../csv/3_ph_stedin_trafo_Hy_OC.csv", Hy_mat)
# H_mat = DataFrame(hcat(H...), :auto)
# CSV.write((@__DIR__) * "/../csv/3_ph_stedin_trafo_H_OC.csv", H_mat)
# mag_energy_mat = DataFrame(hcat(mag_energy...), :auto)
# CSV.write((@__DIR__) * "/../csv/3_ph_stedin_trafo_energy_OC.csv", mag_energy_mat)
# elapsed = round((time_ns() - start)/10^9, digits=2)
