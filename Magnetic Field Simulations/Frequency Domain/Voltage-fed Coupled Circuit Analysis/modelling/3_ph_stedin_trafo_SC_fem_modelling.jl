...
function initialise_params(mesh)
    ...
    return sourceperelement, reluctivityperelement, conductivityperelement, omega, coil_voltage, ext_resistance, ext_inductance, z_length, Rp, Rs
end
...
sourceperelement, reluctivityperelement, conductivityperelement, omega, coil_voltage, ext_resistance, ext_inductance, z_length, Rp, Rs = initialise_params(mesh)
...
printstyled("Primary winding loss = "*string(Pwindingp)*" W\n", color = :blue)
printstyled("Secondary winding loss = "*string(Pwindings)*" W\n", color = :blue)
...
fem_future_distribution_grids.Makie_Plots.plot_surface_tri_1e(msh, colors, "3_ph_stedin_trafo_B_SC", @__DIR__)
...
fem_future_distribution_grids.Makie_Plots.plot_surface_tri_1e(msh, colors, "3_ph_stedin_trafo_H_SC", @__DIR__)
...
fem_future_distribution_grids.Makie_Plots.plot_surface_tri_1e(msh, colors, "3_ph_stedin_trafo_energy_SC", @__DIR__)
...
fem_future_distribution_grids.Makie_Plots.plot_surface_tri_1e(msh, colors, "3_ph_stedin_trafo_Jel_SC", @__DIR__)
elapsed = round((time_ns() - start)/10^9, digits=2)
...
start = time_ns()
fem_future_distribution_grids.Save_VTK.save_vtk(mesh, imag(u[1:mesh.nnodes]), norm.(B), norm.(H), norm.(mag_energy), reluctivityperelement, norm.(Jel), "3_ph_stedin_trafo_SC", @__DIR__)
elapsed = round((time_ns() - start)/10^9, digits=2)
