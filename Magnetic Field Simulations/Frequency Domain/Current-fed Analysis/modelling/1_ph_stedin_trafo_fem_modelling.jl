# #######################################################################################
# FEM Modelling of the STEDIN Transformer (1 Phase)
# Rahul Rane
# #######################################################################################
...
function initialise_params(mesh)
    ...
    sourcefunction(group_id, Jp, Js) = 0 * (1 * (group_id==5) - 1 * (group_id==6)) + 
...
fem_future_distribution_grids.Makie_Plots.plot_surface_tri_1e(msh, colors, "1_ph_stedin_trafo_B", @__DIR__)
...
fem_future_distribution_grids.Makie_Plots.plot_surface_tri_1e(msh, colors, "1_ph_stedin_trafo_H", @__DIR__)
...
fem_future_distribution_grids.Makie_Plots.plot_surface_tri_1e(msh, colors, "1_ph_stedin_trafo_energy", @__DIR__)
elapsed = round((time_ns() - start)/10^9, digits=2)
...
start = time_ns()
fem_future_distribution_grids.Save_VTK.save_vtk(mesh, norm.(u), norm.(B), norm.(H), norm.(mag_energy), reluctivityperelement, "1_ph_stedin_trafo", @__DIR__)
elapsed = round((time_ns() - start)/10^9, digits=2)
