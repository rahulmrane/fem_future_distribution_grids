module Makie_Plots
    ...
    using Colors
    using ColorSchemes
    export get_cell_mesh, plot_surface, plot_mesh

    # Obtain a GeometryBasics.Mesh object suitable for plotting with Makie
    function get_cell_mesh(cell_val, gmsh)
    ...
    function plot_surface(cell_val, gmsh)
        msh, colors = get_cell_mesh(cell_val, gmsh)
    ...
    function plot_mesh(cell_val, gmsh)
        msh, colors = get_cell_mesh(cell_val, gmsh)
        cmap = ColorScheme([RGB(0.788, 0.788, 0.784), RGB(0.592, 0.180, 0.996), RGB(1.0, 0.631, 0.216), RGB(0.671, 1.0, 0.224)])
        f, ax, pl = mesh(msh, color = colors, colormap = cmap, shading = false)
